...
#include "article.h" // module header for this file
...
void writeVTKResultsForSolids(plb::MultiBlockLattice2D<T,DESCRIPTOR>& lattice, T*** uStart, T*** strainStart, T*** stressStart, T*** divStressStart, T*** uBackwardStart, T*** uSpeedBackwardStart, plb::plint iter, T DeltaX, T DeltaT, T InvPhysicalFactorForSpeed, T moduleYoung, T poissonRatio, T rho)
{
    // time and space
    T dx = DeltaX;
    T dt = DeltaT;
    ...
    plb::VtkImageOutput2D<T> vtkOut(plb::createFileName("sldarticle", iter, 8), dx);
...
/// INPUTS ///
    ...
    //////////////////////// main inputs ////////////////////////
    const T DeltaT=0.000001; // physical step: can't be too small because of double precision: otherwise it can't affect distribution functions
    const T maxT=0.05; // max study physical time in seconds, please adjust for static equilibrium convergence
    int nodesPerMeter=200; // number of nodes per meter
    ...
    // note: omega=1.0 or omega=0.01: same results because stress divergence makes a balance between them
    // note: omega must be non-zero to avoid arithmetics bug with palabos library: to be investigated: need a minimum relaxation to obtain static equilibrium?
    // note: omega has a slight influence, it can damp streamed body forces, but the result remains almost the same: to be investigated
    // note: when omega << 1.0, D2Q9 stencil behavior can appear: to be investigated: numerical errors?
    // note: omega seems to does not affect the convergence speed towards solid static equilibrium
    const T omega=0.01; // solid relaxation parameter
    ...
    //////////////////////// material properties ////////////////////////
    std::array<double, 5> youngValues{{200, 2, 0.02, 0.0002, 0.000002}};
    std::array<double, 7> poissonValues{{-0.99, -0.5, 0.0, 0.15, 0.25, 0.35, 0.49}}; // 0.49 is not accurate due to scheme issues or boundary conditions: to be investigated
    T rho=1000; // density kg/m3
    // used convergence factors: [v=0.49:0.000005; v=-0.99:0.000001; others=0.00001]*10^(0) for E=200Gpa, *10^(2) for E=2Gpa and so on (but not<>1 if you want the right force intensity)
    // note: convergenceForceFactor can't be too small because of double precision: otherwise it can't affect distribution functions
    T convergenceForceFactor=0.00001; // intensity of the force, please adjust for static equilibrium convergence and for scheme stability
    T moduleYoung=youngValues[0]*pow(10.0, 9.0); // N/m2
    T poissonRatio=poissonValues[3]; // without units

    //////////////////////// boundary conditions values and options ////////////////////////
    int includedMinXOrY=50; // min condition application
    int excludedMaxXOrY=150; // max condition application
    ...
    bool gaussHermite=true; // true for gauss hermite distribution and related dynamic
...
/// SOLID LATTICE BOLTZMANN LOOP ///
    ...
    for (plint iT=0; iT*DeltaT<maxT; ++iT) {
        ...
        if (iT%savestatIter==0) {
            ...
            writeVTKResultsForSolids(lattice, uStart, strainStart, stressStart, divStressStart, uBackwardStart, uSpeedBackwardStart, iT, DeltaX, DeltaT, 1.0/physicalFactorForSpeed, moduleYoung, poissonRatio, rho);
        }
    ...
    pcout << "Young's modulus: " << moduleYoung << " N/m2" << endl;
    pcout << "Poisson's ratio: " << poissonRatio << " without units" << endl;
