...
/// Add a force term for vlasov dynamics
static void addGravityLikeForce( Cell<T,Descriptor>& cell)
{
    externalForceTemplatesImpl<T, Descriptor<T> >
        ::addGravityLikeForce(cell.getRawPopulations(), cell.getExternal(0));
}
...
/// Vlasov dynamics à terminer et tester et enlever ce qui est inutile
static void addGravityLikeForce( Array<T,Descriptor::q>& f, T* externalScalars )
{
    ...
    {
        T c_f = T();
        for (int iD=0; iD<Descriptor::d; ++iD) { // peu etre que les vitesses :c[iPop][iD] ont deja un certain facteur propre à palabos ce qui concorde avec bonnefoy 1.52
            c_f += Descriptor::c[iPop][iD]*force[iD]; // Fi scalaire ci = cf produit scalaire ici
        }
        f[iPop] += c_f * Descriptor::t[iPop] * Descriptor::invCs2; // 3*cf*Wi avew Wi=Descriptor::t[iPop]
    }
}
