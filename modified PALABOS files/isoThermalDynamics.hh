...
/* *************** Class VlasovDynamics *********************************************** */

template<typename T, template<typename U> class Descriptor>
int VlasovDynamics<T,Descriptor>::id =
    meta::registerGeneralDynamics<T,Descriptor,VlasovDynamics<T,Descriptor> >("Vlasov");

/** \param omega_ relaxation parameter, related to the dynamic viscosity
 */
template<typename T, template<typename U> class Descriptor>
VlasovDynamics<T,Descriptor>::VlasovDynamics(T omega_ )
    : IsoThermalBulkDynamics<T,Descriptor>(omega_)
{ }

template<typename T, template<typename U> class Descriptor>
VlasovDynamics<T,Descriptor>::VlasovDynamics(HierarchicUnserializer& unserializer)
...
template<typename T, template<typename U> class Descriptor>
VlasovDynamics<T,Descriptor>* VlasovDynamics<T,Descriptor>::clone() const {
    return new VlasovDynamics<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor>
int VlasovDynamics<T,Descriptor>::getId() const {
    return id;
}

template<typename T, template<typename U> class Descriptor>
void VlasovDynamics<T,Descriptor>::collide (
        ...
        BlockStatistics& statistics )
{
    ...
    Array<T,Descriptor<T>::d> j;
    momentTemplates<T,Descriptor>::get_rhoBar_j(cell, rhoBar, j);
    T uSqr = dynamicsTemplates<T,Descriptor>::bgk_ma2_collision_vlasov(cell, rhoBar, j, this->getOmega()); // implementation of bgk_ma2_collision_vlasov dans dynamicsTemplates.h
...
template<typename T, template<typename U> class Descriptor>
void VlasovDynamics<T,Descriptor>::collideExternal (
...
template<typename T, template<typename U> class Descriptor>
T VlasovDynamics<T,Descriptor>::computeEquilibrium(plint iPop, T rhoBar, Array<T,Descriptor<T>::d> const& j,
...
template<typename T, template<typename U> class Descriptor>
void VlasovDynamics<T,Descriptor>::decomposeOrder0 (
        Cell<T,Descriptor> const& cell, std::vector<T>& rawData ) const
{
    T rhoBar;
    Array<T,Descriptor<T>::d> j;
...
template<typename T, template<typename U> class Descriptor>
void VlasovDynamics<T,Descriptor>::recomposeOrder0 (
        Cell<T,Descriptor>& cell, std::vector<T> const& rawData ) const
{
    T rhoBar = rawData[0];
    Array<T,Descriptor<T>::d> j;
        ...
        BlockStatistics& statistics )
{
    ...
    Array<T,Descriptor<T>::d> j;
    momentTemplates<T,Descriptor>::get_rhoBar_j(cell, rhoBar, j);
    T uSqr = dynamicsTemplates<T,Descriptor>::bgk_ma2_collision(cell, rhoBar, j, this->getOmega()); // cell, rho et vitesses j avec omega je pense
    if (cell.takesStatistics()) {
        gatherStatistics(statistics, rhoBar, uSqr);
    }
}
